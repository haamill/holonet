...
def get_psnr_ssim(recon_amp, target_amp, multichannel=False):
    ...
    # For scikit-image >= 0.19, use channel_axis instead of multichannel
    ssim_kwargs = {'channel_axis': -1} if multichannel else {}

    # amplitude
    psnrs['amp'] = psnr(target_amp, recon_amp)
    ssims['amp'] = ssim(target_amp, recon_amp, data_range=target_amp.max() - target_amp.min(), **ssim_kwargs)

    # linear
    target_linear = target_amp**2
    recon_linear = recon_amp**2
    psnrs['lin'] = psnr(target_linear, recon_linear)
    ssims['lin'] = ssim(target_linear, recon_linear, data_range=target_linear.max() - target_linear.min(), **ssim_kwargs)
    ...
    ssims['srgb'] = ssim(target_srgb, recon_srgb, data_range=target_srgb.max() - target_srgb.min(), **ssim_kwargs)
